...
def train_ChineseNet():
    ...
    if not os.path.exists("chineseNet1.pkl"):
        joblib.dump(ccNet, 'chineseNet1.pkl')
    else:
        os.remove("chineseNet1.pkl")
        joblib.dump(ccNet, 'chineseNet1.pkl')

# 识别
def chinesePredict(x):
    ccNet = joblib.load('chineseNet1.pkl')
    y = ccNet.predict(x)
    index = np.argmax(y)
    print("汉字识别网络识别结果：{}".format(PROVINCES[index]))
    return y,PROVINCES[index]

if __name__ == "__main__":
    train_ChineseNet()
    print('min_batch版本汉字训练结果完成！')