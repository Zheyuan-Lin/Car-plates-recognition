...
def getData(PATH):
    ...
    list_label = []
    k = 0
    for root, dirs, files in os.walk(os.path.join(PATH, "letters")):
        if len(os.path.basename(root)) > 1:  # startswith() 方法用于检查字符串是否是以指定子字符串开头
            continue
        index = letters_class.index(os.path.basename(root)) # 获得种类对应的索引
...
# 显示图片
def showImg(list_data,x_name,y_name):
...
if __name__ == "__main__":
    ...
    ccNet = TwoLayerNet(input_size = 400,hidden_size = 20,output_size = 24)
    batch_size = 100
    ...
    # 显示训练准确度图
    showImg(train_acc_list,"x","accuracy")
    # 显示损失函数值
    showImg(train_loss_list,"x","L(y,t)")
    print("程序运行时间:{}".format(end_time - begin_time))

    if not os.path.exists("letterNet.pkl"):
        joblib.dump(ccNet,'letterNet.pkl')
    else:
        os.remove("letterNet.pkl")
        joblib.dump(ccNet,'letterNet.pkl')
